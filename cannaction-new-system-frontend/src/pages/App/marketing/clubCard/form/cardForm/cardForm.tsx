import { Autocomplete, FormControl, Grid, IconButton, InputAdornment, InputLabel, OutlinedInput, TextField } from '@mui/material';
...
export const CardForm: React.FC<Props> = ({
	...
	const {
		...
		userTypeLogged === UserTypeEnum.SUPER ? countryId : userCountry
	);
	...
	useEffect(() => {
		if (userTypeLogged === UserTypeEnum.MARKETING) {
			setCountryId(userCountry);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [userTypeLogged]);
		...
		>
			<Grid item xs={5.5}>
				<TextField
					...
					label={t('marketing.addCard.codeInput')}
			...
			{userTypeLogged === UserTypeEnum.SUPER && (
				<Grid item xs={5.5}>
					<Autocomplete
						...
						onChange={(_, newValue) => setCountryId(newValue?.value)}
						renderInput={(params) => (
							<TextField
								{...params}
								variant="outlined"
								...
								label={t('marketing.targets.CountryDropDown')}
							/>
						)}
					/>
				</Grid>
			...
			{countryId && (
				<Grid item xs={5.5}>
					<Autocomplete
						...
						sx={style}
						options={data}
						value={data.find((s) => s.value == storeId) || null}
						getOptionLabel={(option) => option.label}
						isOptionEqualToValue={(option, value) => option.value === value.value}
						onChange={(_, newValue) => {
							if (newValue) {
								setStoreId && setStoreId(newValue?.value);
							}
						}}
						renderInput={(params) => (
							<TextField
								{...params}
								variant="outlined"
								placeholder={t('marketing.addCard.storeDropdown')}
								label={t('marketing.addCard.storeDropdown')}
							/>
						)}
					/>
				</Grid>
