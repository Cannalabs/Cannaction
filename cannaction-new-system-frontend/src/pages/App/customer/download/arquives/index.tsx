...
export const ArchivesCustomer: React.FC = () => {
	const { id } = useParams();
		...
		<Grid container height={'259.61px'}>
			<DataTable
				...
				rows={data?.data.map((arc) => ({
					...
					userType: arc.userType
						? getUserTypeText(arc.userType)
						: t('store.customers.archives.allUsers'),
					country: arc.country
						? arc.country.name
						: t('store.customers.archives.allCountries'),
					actions: (
						<>
