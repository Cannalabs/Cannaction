...
const KEY = 'checked-coupons-store';

export const useCheckedCouponsStore = (filter?: PaginationFilterCouponRequest) => {
	const { data, isLoading, isRefetching, refetch } = useQuery(
		[KEY, filter],
		async () => {
			return CouponService.findCheckedCouponsUserStore(filter);
		},
		{
